    ...
    private async Task CreateSalesTransactionAsync(Domain.Entities.Booking booking)
    {
        try
        {
            ...
            var salesRequest = new
            {
                ...
                customerName = $"Customer-{booking.CustomerId.ToString().Substring(0, 8)}"
            };

            var response = await httpClient.PostAsJsonAsync("/api/Sales", salesRequest);
            
            if (!response.IsSuccessStatusCode)
            {
                // Log error but don't fail the booking confirmation
                Console.WriteLine($"Failed to create sales transaction for booking {booking.BookingId}: {response.StatusCode}");
            }
        }
        catch (Exception ex)
        {
            // Log error but don't fail the booking confirmation
            Console.WriteLine($"Error creating sales transaction for booking {booking.BookingId}: {ex.Message}");
        }
    }
