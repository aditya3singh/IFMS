...
using IFMS.Sales.Infrastructure.Repositories;
...
var stationConnection = builder.Configuration.GetConnectionString("StationConnection")
    ?? throw new InvalidOperationException(
        "ConnectionStrings:StationConnection is required (IFMS_StationDB) for dealer-scoped sales.");
...
builder.Services.AddScoped<IFMS.Sales.Application.Interfaces.IComplaintRepository, IFMS.Sales.Infrastructure.Repositories.ComplaintRepository>();
builder.Services.AddScoped<IFMS.Sales.Application.Commands.ComplaintCommandHandler>();

// Add HttpContextAccessor for accessing current request context
builder.Services.AddHttpContextAccessor();

// Add HttpClient for Inventory API communication with authorization forwarding
builder.Services.AddTransient<IFMS.Sales.API.Http.AuthorizationHeaderHandler>();
builder.Services.AddHttpClient("InventoryAPI", client =>
{
    var inventoryApiUrl = builder.Configuration["Services:InventoryAPI"] ?? "http://localhost:5004";
    client.BaseAddress = new Uri(inventoryApiUrl);
    client.Timeout = TimeSpan.FromSeconds(30);
})
.AddHttpMessageHandler<IFMS.Sales.API.Http.AuthorizationHeaderHandler>();
...
builder.Services.AddScoped<ISalesNotificationPublisher, HttpSalesNotificationPublisher>();
