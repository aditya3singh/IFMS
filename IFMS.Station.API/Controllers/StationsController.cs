...
public class StationsController : ControllerBase
{
    ...
    public async Task<IActionResult> AssignDealer(Guid id, [FromBody] AssignDealerDto dto)
    {
        try
        {
            ...
            // Check if station already has an assignment
            if (await _dealerAssignmentRepository.StationHasAssignmentAsync(id))
            {
                _logger.LogWarning("Station {StationId} already has an assigned dealer", id);
                return BadRequest(new { error = "Station already has an assigned dealer" });
            }
