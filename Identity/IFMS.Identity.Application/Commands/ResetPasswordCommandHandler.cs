...
public class ResetPasswordCommandHandler
{
    ...
    public ResetPasswordCommandHandler(IUserRepository users, IOtpChallengeRepository otps)
    {
        _users = users;
        _otps = otps;
    }

    public async Task HandleAsync(ResetPasswordRequest request)
    {
        if (string.IsNullOrWhiteSpace(request.NewPassword) || request.NewPassword.Length < 8)
            throw new InvalidOperationException("Password must be at least 8 characters.");

        var key = AuthIdentifier.ToOtpStorageKey(request.Identifier);
        var row = await _otps.GetLatestAsync(key, OtpPurposes.PasswordReset)
